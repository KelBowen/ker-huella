...
seed_plants <- plants_input %>%
  transmute(
    ...
    english_name = str_trim(english_name),
    
    french_name = str_trim(french_name),
    
    native = case_when(
      native %in% c(TRUE, "TRUE", "True", "Oui", "oui") ~ TRUE,
      native %in% c(FALSE, "FALSE", "False", "Non", "non") ~ FALSE,
      TRUE ~ NA
    ),
...
cat(
  "Plant-location links:",
  nrow(plant_locations_final),
  "\n"
)
